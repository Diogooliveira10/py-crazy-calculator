...
class Calculator2:
    ...
    def calculate(self, request: FlaskRequest) -> Dict: # type: ignore
        ...
        formated_response = self.__forma_response(calculated_number)
        return formated_response

    def __validate_body(self, body: Dict) -> List[float]:
        if "numbers" not in body:
            raise HttpUnprocessableEntityError("Poorly formatted body!")
    ...
    def __forma_response(self, calculated_number: float) -> Dict:
        return {
            "data": {
                "Calculator": 2,
